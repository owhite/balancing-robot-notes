...
def calculate_total_inertia_from_mass(
    ...
    for name, part in assembly_data["meshes"].items():
        ...
        # --- inertia accumulation (unchanged behavior) ---
...
def compute_lqr_for_balancing_bot(params, m_body, I_body, com_body_mm, m_wheel, axle_origin_mm, axle_vector):
    ...
    l = np.linalg.norm(r_perp) * 1e-3

    r = params["wheel_radius"] * 1e-3
...
def main():
    ...
    # total CoM refers to the whole device, wheels and rotors included
    print("\n=== CoM SUMMARY ===")
    ...
    axle_origin_mm = np.array(params["axle_origin"])
    axle_vector = np.array(params["axle_vector"])
