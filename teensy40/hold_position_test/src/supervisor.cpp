...
void controlLoop(MPU6050 &imu, Supervisor_typedef *sup,
  ...
  // TO RUN:
  // Use channel 2 (throttle) on the RC transmitter
  //  pull back to go into holding mode
  //  push forward to let the motor spin freely
  // Put your hand on motor, try to turn it should resist
  if (sup->rc_raw[1].raw_us < 1400) {
    if (!holding) {
  ...
  else if (sup->rc_raw[1].raw_us > 1600) {
    hold_pos_rad = sup->esc[0].state.pos_rad;
    holding = false;
  }
  else {
  ...
  if (holding) {
    ...
    // - Tapered deadband logic is applied
    // Creates a "magnetic detent" feel: free in the center, stiff edges

    // --- Parameters ---
    const float Kspring    = 0.8f;   // Nm/rad, stiffness outside deadband
    const float B          = 0.02f;  // Nm/(rad/s), damping
    const float DEAD_BAND  = 0.001f;  // rad (~0.6°)

    float cmd_torque = 0.0f;

    // --- Tapered deadband logic ---
    if (fabs(pos_err) < DEAD_BAND) {
      // scale down torque linearly inside deadband
      float scale = fabs(pos_err) / DEAD_BAND;  // 0..1
      cmd_torque = (Kspring * pos_err * scale) - B * vel;
    } else {
      // full spring-damper outside
