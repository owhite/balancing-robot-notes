...
struct IMU_typedef {
  ...
  float roll, pitch, yaw;  // Orientation angles in radians
...
enum SupervisorMode {
  ...
  SUP_MODE_SINUSOIDAL,
  SUP_MODE_INERTIA_TEST,
  SUP_MODE_SET_POSITION,
  SUP_MODE_MOVE_TO_POSITION,
  SUP_MODE_SIN_TORQUE,
...
struct Supervisor_typedef {
  ...
  float user_theta = M_PI;
  float user_Kth_term = 0.0f;
  float user_Kw_term = 0.0f;
  float user_Ki_term = 0.0f; 
  float user_pulse_torque   = 0.2f;    // amplitude of torque pulse
  float user_torque   = 0.2f;    // amplitude of torque pulse
